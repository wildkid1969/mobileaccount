...
<%@ page contentType="text/html; charset=UTF-8"%>
	...
	<meta name="Keywords" content="信息修改页"/>
	<meta name="Description" content="信息修改页"/>
	<title>信息修改页</title>
		...
		function load(){
			...
			eBind(document.getElementById("save-btn"), "click", function(){
				...
				$.ajax({
					type : "post",
					async : true,
					url : requestURL+"/mobileaccount/call/updatecorpinfo",
					data: {
						corpid : corpid,
					...
					success : function(json, textStatus){
						window.location.href = "./info_read.html?wd=" + encodeURIComponent(word) + "&corpid=" + corpid + "&pageNum=" + pageNum;
					},
					error : function(XMLHttpRequest, textStatus, errorThrown){
						alert("更新企业信息失败。[readyState:"+XMLHttpRequest.readyState+", textStatus:"+textStatus+"]");
					}
				});
			});
			...
			$.ajax({
				...
				success : function(json, textStatus){
					...
					for(var i=0; i<connects.length-1; i++){
						...
						document.getElementById("contacts").innerHTML += "<div class=\"item\"><label>电话号码：</label><span><input type=\"text\" value=\""+connect[1]+"\" /></span><label>手机号码：</label><span><input type=\"text\" value=\""+connect[2]+"\" /></span><label>联系人姓名：</label><span><input type=\"text\" value=\""+connect[0]+"\" /></span></div>";
					}

					document.getElementById("enterpriseType").value = json.enterpriseType;
					document.getElementById("province").value = json.province;
					document.getElementById("city").value = json.city;
					document.getElementById("organizationcode").value = json.organizationcode;
					document.getElementById("brandname").value = json.brandname;

					document.getElementById("legalperson").value = json.legalperson;
					document.getElementById("registeredcapital").value = json.registeredcapital;
					document.getElementById("currencytype").value = json.currencytype;
					document.getElementById("createTime").value = json.createTime;
					document.getElementById("logindate").value = json.logindate;
					document.getElementById("businesslicensecode").value = json.businesslicensecode;
					document.getElementById("address").value = json.address;
					document.getElementById("keepphone").value = json.keepphone;

					document.getElementById("mainproducts").value = json.mainproducts;
					document.getElementById("mainarea").value = json.mainarea;
					document.getElementById("businessaddress").value = json.businessaddress;
					document.getElementById("specialmarket").value = json.specialmarket;
					document.getElementById("areaid").value = json.areaid;
					document.getElementById("operatingstatus").value = json.operatingstatus;
					document.getElementById("introduce").value = json.introduce;

					document.getElementById("datagrade").value = json.datagrade;
				},
				error : function(XMLHttpRequest, textStatus, errorThrown){
...
<body>
	...
	<div class="infomation-panel">
		<span id="corpname"></span><button id="save-btn">保存</button>
		<h2>基本信息</h2>
		...
		<div id="registered-info" class="items">
			<div class="item">
				...
				<label>成立日期：</label><span><input id="createTime" type="text" value="" placeholder="请输入成立日期(格式：YYYY/MM/DD)" /></span>
				<label>注册时间：</label><span><input id="logindate" type="text" value="" placeholder="请输入注册时间(格式：YYYY/MM/DD)" /></span>
